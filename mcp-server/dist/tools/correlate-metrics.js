...
exports.name = "correlate_metrics";
exports.description = "Analyze correlations between commerce and behavioral metrics over the last N days for a client. Detects patterns such as revenue vs. friction score, traffic vs. conversions, rage clicks vs. conversion rate, and more.";
exports.inputSchema = zod_1.z.object({
    client_id: zod_1.z.number().int().describe("The client ID to analyze correlations for"),
    days: zod_1.z
        .number()
        .int()
        .min(3)
...
async function execute(input) {
  ...
  `;
    const result = await db_js_1.default.query(query, [client_id, days]);
    ...
    if (rows.length < 3) {
        return {
            ...
            message: "Insufficient data points (need at least 3 days) to calculate meaningful correlations.",
            correlations: [],
        };
    }
    const pairs = [
    ...
    const correlations = pairs.map(({ metricA, metricB, hypothesis }) => {
        const a = rows.map((r) => r[metricA]);
        const b = rows.map((r) => r[metricB]);
        const r = pearson(a, b);
        return {
            metric_a: metricA,
            metric_b: metricB,
...
//# sourceMappingURL=correlate-metrics.js.map